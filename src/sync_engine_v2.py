"""New Sync Engine V2 using Phase 6 Architecture

This version uses:
- StateManagerV2: 3-table schema with proper relationships
...
class SyncEngineV2:
    ...
    def __init__(
        ...
        """
        Initialize SyncEngineV2 with Phase 6 components

        Args:
            paprika_client: Configured Paprika API client
            skylight_client: Configured Skylight API client
            state_manager: StateManagerV2 for 3-table architecture
        ...
        logger.info(f"SyncEngineV2 initialized for '{paprika_list_name}' ↔ '{skylight_list_name}'")
    ...
    def sync(self, dry_run: bool = False) -> Dict[str, Any]:
        ...
        logger.info(f"Starting SyncEngineV2 sync (dry_run={dry_run})")
        ...
        try:
            ...
            logger.info(f"SyncEngineV2 completed successfully in {sync_duration:.2f}s")
            ...
            logger.error(f"SyncEngineV2 sync failed: {e}")
            sync_duration = (datetime.now(timezone.utc) - sync_start).total_seconds()
            return {
                'dry_run': dry_run,
                'sync_start': sync_start,
    ...
    def get_sync_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'architecture_version': 'Phase 6 (StateManagerV2 + ItemLinker + ConflictResolver)',
